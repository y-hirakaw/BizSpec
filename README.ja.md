...
> ⚠️ **現在、方向修正の真っ最中です（2026年5月）。**
> 当初の「全プロセスを YAML に分解する」アプローチは絞り込み中で、
> 新しい方針は **CSV ベースの棚卸し（`bizspec/portfolio.csv`）でレバレッジを判別し、AI 実行可能な仕様に昇格させる unit だけを YAML 化する** という二段構えです。
> 以下のスクリーンショットやスキル説明は YAML-first 時代の設計を反映しており、ドッグフーディングで新方針が検証された段階で書き直します。
> 経緯と判断材料は [`BizSpec_設計メモ_2026-05-08.md`](BizSpec_設計メモ_2026-05-08.md) を参照してください。
